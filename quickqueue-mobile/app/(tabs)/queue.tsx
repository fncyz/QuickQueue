...
export default function QueueScreen() {
  ...
  return <SafeAreaView style={[s.safe, { backgroundColor: colors.background }]} edges={['top']}><Animated.ScrollView style={[s.page, { backgroundColor: colors.background }]} contentContainerStyle={s.content} showsVerticalScrollIndicator={false} onScroll={coverHeader.onScroll} scrollEventThrottle={16}>
    ...
    {!appointment ? <View style={[s.empty, { backgroundColor: colors.surface, borderColor: colors.border }]}><Ionicons name="people-outline" size={48} color={colors.accent} /><Text style={[s.emptyTitle, { color: colors.text }]}>No active appointment</Text><Text style={[s.emptyCopy, { color: colors.muted }]}>Book an appointment to view your live queue position here.</Text><Pressable onPress={() => router.push('/booking')} style={s.book}><Text style={s.bookText}>Book Now</Text></Pressable></View> : <>
      <View style={[s.card, { backgroundColor: colors.surface, borderColor: colors.border }]}><SectionTitle icon="people-circle" title="Your Queue Status" subtitle="Real-time updates on your queue." right="You’re all set!" /><View style={[s.ticketPanel, { backgroundColor: colors.surfaceAlt }]}><Metric label="NOW SERVING" value={appointment.now_serving} icon="megaphone-outline" detail="Please proceed to the service counter." /><Metric label="YOUR NUMBER" value={appointment.queue_number} icon="person-outline" detail={appointment.people_ahead ? `${appointment.people_ahead} people ahead of you` : 'You are next in line'} center /><Metric label="ESTIMATED WAITING TIME" value={`${appointment.estimated_wait} minutes`} icon="time-outline" detail="Please wait for your turn." /></View><View style={[s.note, { backgroundColor: colors.iconBackground }]}><Ionicons name="notifications-outline" size={14} color={colors.accent} /><Text style={[s.noteText, { color: colors.muted }]}><Text style={[s.noteStrong, { color: colors.accent }]}>Note: </Text>Waiting time may change based on the number of people being served.</Text></View></View>
      <View style={[s.card, { backgroundColor: colors.surface, borderColor: colors.border }]}><SectionTitle icon="flash" title="Appointment Actions" subtitle="Manage your appointment quickly and easily." /><View style={s.actions}><Pressable disabled={acting} onPress={() => performAction('check_in')} style={[s.action, s.check, { backgroundColor: isDark ? '#142C2A' : 'transparent' }]}><Ionicons name="checkmark-circle" size={24} color="#22A950" /><View style={s.actionCopy}><Text style={s.checkTitle}>Check In</Text><Text style={[s.actionText, { color: colors.muted }]}>Let staff know you’ve arrived.</Text></View><Ionicons name="chevron-forward" size={17} color="#22A950" /></Pressable><Pressable disabled={acting} onPress={confirmCancellation} style={[s.action, s.cancel, { backgroundColor: isDark ? '#301F29' : 'transparent' }]}><Ionicons name="close-circle" size={24} color="#ED1C24" /><View style={s.actionCopy}><Text style={s.cancelTitle}>Cancel Appointment</Text><Text style={[s.actionText, { color: colors.muted }]}>Cancel if you cannot make it.</Text></View><Ionicons name="chevron-forward" size={17} color="#ED1C24" /></Pressable></View></View>
      <View style={[s.card, { backgroundColor: colors.surface, borderColor: colors.border }]}><SectionTitle icon="calendar" title="Appointment Information" subtitle="Here are the details of your appointment." />{[
      ...
      ].map(([icon, label, value]) => <View key={label} style={[s.detail, { borderBottomColor: colors.border }]}><Ionicons name={icon as keyof typeof Ionicons.glyphMap} size={14} color={colors.accent} /><Text style={[s.detailLabel, { color: colors.muted }]}>{label}</Text><Text style={[s.detailValue, { color: colors.text }]}>{value}</Text></View>)}</View>
      <Pressable style={[s.alert, { backgroundColor: colors.surfaceAlt }]}><Ionicons name="notifications-outline" size={17} color={colors.accent} /><Text style={[s.alertTitle, { color: colors.accent }]}>System Alerts</Text><Text style={[s.alertCopy, { color: colors.muted }]}>Get notified when your queue status changes.</Text><Ionicons name="chevron-forward" size={17} color={colors.accent} /></Pressable>
    </>}
  </Animated.ScrollView></SafeAreaView>;
}

function SectionTitle({ icon, title, subtitle, right }: { icon: keyof typeof Ionicons.glyphMap; title: string; subtitle: string; right?: string }) { const { colors } = useAppTheme(); return <View style={s.sectionHead}><View style={s.sectionIcon}><Ionicons name={icon} size={14} color="#FFF" /></View><View style={s.sectionCopy}><Text style={[s.sectionTitle, { color: colors.text }]}>{title}</Text><Text style={[s.sectionSubtitle, { color: colors.muted }]}>{subtitle}</Text></View>{right && <Text style={[s.ready, { color: colors.accent }]}>✓ {right}</Text>}</View>; }
...
const s = StyleSheet.create({
  ...
  ...({
    hero: { ...residentLayout.header, alignItems: 'center', justifyContent: 'center', marginBottom: -34, paddingTop: 44 },
