...
export default function RegistrationCompleteScreen() {
  ...
  return <SafeAreaView style={s.safe} edges={["top", "bottom"]}><SecuritySetupBackdrop /><View style={s.content}><View style={s.iconHalo}><View style={s.icon}><Ionicons name="checkmark" size={49} color="#FFF" /></View></View><Text style={s.title}>Congratulations!</Text><Text style={s.description}>You are all set! Your account has successfully registered.</Text><SecurityGradientButton onPress={backToSignIn} label="Back to Sign In" style={s.button} /></View></SafeAreaView>;
}
