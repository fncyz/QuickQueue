...
export default function SecuritySetupProgress({ current }: { current: number }) {
  return <View style={s.wrap}>{steps.map((label, index) => {
    ...
    return <View key={label} style={s.step}>
      <View style={[s.circle, active && s.circleActive, complete && s.circleComplete]}>{complete ? <Ionicons name="checkmark" size={13} color="#FFF" /> : <Text style={[s.number, active && s.numberActive]}>{number}</Text>}</View>
...
const s = StyleSheet.create({
  ...
  circleComplete: { backgroundColor: "#1AAE63", borderColor: "#1AAE63" },
